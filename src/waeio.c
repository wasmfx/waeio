...
int waeio_main(void* (*main)(void*), void *arg) {
  ...
  int64_t sockfd = 0;
  wasio_fd_t vfd;
  assert(wasio_wrap(&ctl.wfd, sockfd, &vfd) == WASIO_OK);
  ctl.nconns++;
  // Allocate fiber for main.
  fiber_t mainfiber = fiber_alloc(main);
  ctl.fibers[(uint32_t)vfd] = mainfiber;
  // Enqueue main (TODO)
  queue_push(ctl.frontq, (struct fiber_closure){ .fiber = mainfiber, .arg = (void*)(uintptr_t)vfd });
...
int waeio_accept(wasio_fd_t vfd, wasio_fd_t *new_conn) {
  ...
  do {
    ...
    if (wasio_accept(&ctl.wfd, vfd, new_conn) == WASIO_OK)
      return 0;
    else
      return -1;
  } while (is_busy(errno));
...
int waeio_recv(wasio_fd_t vfd, uint8_t *buf, uint32_t len) {
  ...
  do {
    ...
    if (wasio_recv(&ctl.wfd, vfd, buf, len, &recvlen) == WASIO_OK)
      return recvlen;
    else
      return -1;
  } while (is_busy(errno));
...
int waeio_send(wasio_fd_t vfd, uint8_t *buf, uint32_t len) {
  ...
  do {
    ...
    if (wasio_send(&ctl.wfd, vfd, buf, len, &sendlen) == WASIO_OK)
      return sendlen;
    else
      return -1;
  } while (is_busy(errno));
